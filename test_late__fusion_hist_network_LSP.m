...
%     test_Data = (test_Data - train_min) ./ (train_max - train_min);
    test_labels = Test_fea_from_hist_lsp.label;
    ...
    [~, ind] = min(EER_dev);
    Model = model{ind};
    [lbl, acc, dec1_lsp] = predict(dev_labels', sparse(double(dev_Data)), Model);
    [lbl, acc, dec2_lsp] = predict(test_labels', sparse(double(test_Data)), Model);
...
%     test_Data = (test_Data - train_min) ./ (train_max - train_min);
    ...
    [~, ind] = min(EER_dev);
    Model = model{ind};
    [lbl, acc, dec1_cnn] = predict(dev_labels', sparse(double(dev_Data)), Model);
    [lbl, acc, dec2_cnn] = predict(test_labels', sparse(double(test_Data)), Model);
    
    dec1 = dec1_lsp + dec1_cnn;
    dec2 = dec2_lsp + dec2_cnn;
    Late_Fusion_LSP_cnn_hist.dec1.(freStrTemp) = dec1;
    Late_Fusion_LSP_cnn_hist.dec2.(freStrTemp) = dec2;
    Late_Fusion_LSP_cnn_hist.dev_labels.(freStrTemp) = dev_labels;
    Late_Fusion_LSP_cnn_hist.test_labels.(freStrTemp) = test_labels;
    ...
    Late_Fusion_LSP_cnn_hist.HTER.(freStrTemp) = HTER; 
    ...
    Late_Fusion_LSP_cnn_hist.EER.(freStrTemp) = EER; 
    fprintf('%s: HTER=%d EER=%d \n', freStrTemp, HTER, EER);
end
save('.\results\Late_Fusion_LSP_cnn_hist.mat', 'Late_Fusion_LSP_cnn_hist');
