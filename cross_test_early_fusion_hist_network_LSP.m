...
for fy = 1 : length(frequency)
    ...
    feaPath = [fullfile(cross_fea_path, freStr), '\'];
    Hist = load([feaPath, 'train_LCBP_process.mat']);
...
%         end
    ...
    Hist = load([feaPath, 'train_hist.mat']);
    Cross_Hist_fea_train = Hist.Hist_fea;
    max_value = max(Cross_Hist_fea_train(:));
    min_value = min(Cross_Hist_fea_train(:));
    Cross_Hist_label_train = Hist.Hist_label;
    Hist = load([feaPath, 'devel_hist.mat']);
    Cross_Hist_fea_devel = Hist.Hist_fea;
    Cross_Hist_label_devel = Hist.Hist_label;
...
%         end
    ...
    Cross_train_Data = [Cross_train_fea_lsp Cross_train_fea_cnn];
    train_max = repmat(train_max_temp, size(Cross_train_Data, 1), 1);
    train_min = repmat(train_min_temp, size(Cross_train_Data, 1), 1);
    Cross_train_Data = (Cross_train_Data - train_min) ./ (train_max - train_min);
    ...
    Cross_dev_Data = [Cross_dev_fea_lsp Cross_dev_fea_cnn];
    train_max = repmat(train_max_temp, size(Cross_dev_Data, 1), 1);
    train_min = repmat(train_min_temp, size(Cross_dev_Data, 1), 1);
    Cross_dev_Data = (Cross_dev_Data - train_min) ./ (train_max - train_min);
    ...
    Cross_test_Data = [Cross_test_fea_lsp Cross_test_fea_cnn];
    train_max = repmat(train_max_temp, size(Cross_test_Data, 1), 1);
    train_min = repmat(train_min_temp, size(Cross_test_Data, 1), 1);
    Cross_test_Data = (Cross_test_Data - train_min) ./ (train_max - train_min);
    ...
    [~, ind] = min(EER_dev);
    Model = model{ind};
    [lbl, acc, dec] = predict(dev_labels', sparse(double(dev_Data)), Model);

    % cross test
    [lbl, acc, dec1] = predict(Cross_train_labels', sparse(double(Cross_train_Data)), Model);
    [lbl, acc, dec2] = predict(Cross_dev_labels', sparse(double(Cross_dev_Data)), Model);
    [lbl, acc, dec3] = predict(Cross_test_labels', sparse(double(Cross_test_Data)), Model);
    
    Cross_Fusion_LSP_cnn_hist.dec.(freStrTemp) = dec;
    Cross_Fusion_LSP_cnn_hist.dec1.(freStrTemp) = dec1;
    Cross_Fusion_LSP_cnn_hist.dec2.(freStrTemp) = dec2;
    Cross_Fusion_LSP_cnn_hist.dec3.(freStrTemp) = dec3;
    
    Cross_Fusion_LSP_cnn_hist.dev_labels.(freStrTemp) = dev_labels;
    Cross_Fusion_LSP_cnn_hist.Cross_train_labels.(freStrTemp) = Cross_train_labels;
    Cross_Fusion_LSP_cnn_hist.Cross_dev_labels.(freStrTemp) = Cross_dev_labels;
    Cross_Fusion_LSP_cnn_hist.Cross_test_labels.(freStrTemp) = Cross_test_labels;
    ...
    Cross_Fusion_LSP_cnn_hist.HTER_train.(freStrTemp) = HTER_train;
    Cross_Fusion_LSP_cnn_hist.HTER_dev.(freStrTemp) = HTER_dev;
    Cross_Fusion_LSP_cnn_hist.HTER_test.(freStrTemp) = HTER_test;
    fprintf('%s: train_HTER=%s dev_HTER=%s test_HTER=%s\n', freStrTemp, num2str(HTER_train), num2str(HTER_dev), num2str(HTER_test));
end
save('.\results\Cross_Fusion_LSP_cnn_hist.mat', 'Cross_Fusion_LSP_cnn_hist');
